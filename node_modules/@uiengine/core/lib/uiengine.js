...
const optionWithDefault = (def, value) => typeof value !== 'undefined' ? value : def;
...
const startWatcher = (state, opts, server) => {
  ...
  const watcher = chokidar.watch(watchFiles, watchOptions).on('add', filePath => handleFileChange(filePath, 'added')).on('addDir', filePath => handleFileChange(filePath, 'added')).on('change', filePath => handleFileChange(filePath, 'changed')).on('unlink', filePath => handleFileChange(filePath, 'deleted')).on('unlinkDir', filePath => handleFileChange(filePath, 'deleted'));
...
const startServer = (state, opts) => {
  ...
  server.init(options, (err, instance) => {
    ...
    server.watch([_paths(pagesPattern), _paths(tokensPattern), _paths(variantsPattern)]).on('change', filePath => {
...
const theo = exports.theo = (theo, options = {}) => require('./integrations/theo')(theo, options);

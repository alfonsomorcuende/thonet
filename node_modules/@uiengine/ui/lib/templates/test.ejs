  ...
  <body>
    ...
    <%_ if (type === 'category' && tokens && tokens.length) { _%>
      <%_ Object.values(tokens).forEach(category => { _%>
        <section class="uie-tokens__category">
      ...
      <table class="uie-thonet-tokens__container uie-thonet-tokens__container--<%= type %>" style="margin-bottom: 100px;">
        ...
        <tbody>
          <%_ Object.values(tokens).forEach(token => { _%>
            <%_ const color = token.value _%>
          <tr class="uie-thonet-color-token">
            <td><%= token.name %></td>
            <% if (token.variable) { %>
    ...
    <%_ } else if (type === 'font') { _%>
      <div class="uie-tokens__container uie-tokens__container--<%= type %>">
        <%_ Object.values(tokens).forEach(token => { _%>
          <dl class="uie-font-token">
            <% if (token.name) { %>
              <div class="uie-font-token__info">
                <dt class="uie-font-token__label">Font Face</dt>
                <dd class="uie-font-token__value"><%= token.name %></dd>
              </div>
            <% } %>
            <% if (token.fontweight) { %>
              <div class="uie-font-token__info" >
                <dt class="uie-font-token__label">Font Weight</dt>
                <dd class="uie-font-token__value"><%= token.fontweight %></dd>
              </div>
            <% } %>
            <% if (token.variable) { %>
              <div class="uie-font-token__info">
                <dt class="uie-font-token__label">Variable</dt>
            ...
            <div class="uie-font-token__info uie-font-token__info--example">
              <dt class="uie-font-token__label">Demo</dt>
            ...
            <% if (token.description) { %>
              <div class="uie-font-token__info">
                <dt class="uie-font-token__label">Description</dt>
                <dd class="uie-font-token__value"><%= token.description %></dd>
              </div>
            <% } %>
            <% if (token.license) { %>
              <div class="uie-font-token__info" >
                <dt class="uie-font-token__label">License</dt>
    ...
    <%_ } else if (type === 'icon') { _%>
      ...
      <table class="uie-tokens__container uie-tokens__container--icon">
        <thead>
          <tr>
            <th class="uie-icon-token__label uie-icon-token__label--icon">Icon</th>
            <th class="uie-icon-token__label uie-icon-token__label--name">Name</th>
            <th class="uie-icon-token__label uie-icon-token__label--value">Usage</th>
            <%_ if (columns.variable) { _%>
              <th class="uie-icon-token__label uie-icon-token__label--variable">Variable</th>
            <%_ } _%>
            <%_ if (columns.description) { _%>
              <th class="uie-icon-token__label uie-icon-token__label--description">Description</th>
            <%_ } _%>
            <%_ if (columns.reference) { _%>
              <th class="uie-icon-token__label uie-icon-token__label--reference">Reference</th>
            <%_ } _%>
          </tr>
        </thead>
        <tbody>
    ...
    <%_ } else { _%>
      ...
      <table class="uie-tokens__container uie-tokens__container--default">
        <thead>
          <tr>
            <th class="uie-default-token__label uie-default-token__label--name">Name</th>
            <th class="uie-default-token__label uie-default-token__label--value">Value</th>
            <%_ if (columns.variable) { _%>
              <th class="uie-default-token__label uie-default-token__label--variable">Variable</th>
            <%_ } _%>
            <%_ if (columns.description) { _%>
              <th class="uie-default-token__label uie-default-token__label--description">Description</th>
            <%_ } _%>
            <%_ if (columns.reference) { _%>
              <th class="uie-default-token__label uie-default-token__label--reference">Reference</th>
            <%_ } _%>
          </tr>
        </thead>
        <tbody>
    ...
    <%_ } _%>
  <%_ } _%>
  <link href="/_assets/styles/uiengine-inject.38be60fb1f1cbb0d970c.css" rel="stylesheet">
  <div class="uie">
